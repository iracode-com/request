...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    ...
    >
        @if (count($containers) || $emptyLabel !== false)
            <div class="table-repeater-container overflow-auto rounded-xl relative ring-1 ring-gray-200 dark:ring-white/20">
                <table class="w-full">
                    <thead @class([
                        'table-repeater-header-hidden sr-only' => ! $renderHeader,
                        'table-repeater-header rounded-t-xl overflow-hidden border-b border-gray-950/5 dark:border-gray-700' => $renderHeader,
                    ])>
                    <tr class="text-xs md:divide-x md:divide-gray-950/5 dark:md:divide-gray-7000">
                        @foreach ($headers as $key => $header)
                            <th
                                    @class([
                                        'table-repeater-header-column p-2 font-medium first:rounded-tl-xl last:rounded-tr-xl bg-gray-100 dark:text-gray-300 dark:bg-gray-800',
                        ...
                        @if ($hasActions && count($containers))
                            <th class="table-repeater-header-column w-px last:rounded-tr-xl p-2 bg-gray-100 dark:bg-gray-800">
                                <span class="sr-only">
                                    {{ trans('Actions') }}
                                </span>
                            </th>
                    ...
                    @else
                        <tr class="table-repeater-row table-repeater-empty-row">
                            <td colspan="{{ count($headers) + intval($hasActions) }}"
                                ...
                                {{ $emptyLabel ?: trans('No results found') }}
                            </td>
                        </tr>
                    @endif
                    </tbody>
