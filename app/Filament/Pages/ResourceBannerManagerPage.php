...
class ResourceBannerManagerPage extends Page
{
    use HasPageShield;
    protected static string $view = 'vendor.banner.pages.banner-manager';

    protected static ?string $slug = 'resource-banner-manager';
    ...
    private function getScopes(): array
    {
        ...
        foreach ($resources as $resource) {
            ...
            $resourceSlug = $resource::getNavigationLabel();
            $resourcePath = str($resource)->value();
            // $scopes[$resourceSlug] = [$resourcePath => Str::afterLast(str($resourcePath), '\\')];
            $scopes[$resourceSlug] = [$resourcePath => $resourcePath::getNavigationLabel()];
    ...
    private function getPagesForResource($resourceClass): array
    {
        ...
        foreach ($resourceClass::getPages() as $page) {
            ...
            if(str_starts_with($pageName,'List')){
                $pageName = 'لیست '.__(pascalToTitle( str_replace('List','',$pageName)));
            }
            else if(str_starts_with($pageName,'Create')){
                $pageName = 'ثبت '.__(pascalToTitle(str_replace('Create','',$pageName)));
            }
            else if(str_starts_with($pageName,'Edit')){
                $pageName = 'ویرایش '.__(pascalToTitle(str_replace('Edit','',$pageName)));
            }
